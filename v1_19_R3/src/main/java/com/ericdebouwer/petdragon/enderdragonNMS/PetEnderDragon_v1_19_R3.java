...
public class PetEnderDragon_v1_19_R3 extends EnderDragon implements PetEnderDragon {
        ...
        try {
            if (!BuiltInRegistries.ENTITY_TYPE.getOptional(mcKey).isPresent()) {
                ...
                Registry.register(BuiltInRegistries.ENTITY_TYPE, PetEnderDragon.ENTITY_ID,
                        EntityType.Builder.of(PetEnderDragon_v1_19_R3::new, MobCategory.MONSTER).noSummon().build(PetEnderDragon.ENTITY_ID));
                BuiltInRegistries.ENTITY_TYPE.freeze();
            }
            // specialsource does ignore reflection, so non mapped names
            jumpField = LivingEntity.class.getDeclaredField("bi");
            jumpField.setAccessible(true);
            checkWalls = EnderDragon.class.getDeclaredMethod("b", AABB.class);
            checkWalls.setAccessible(true);
            checkCrystals = EnderDragon.class.getDeclaredMethod("fS");
    ...
    public PetEnderDragon_v1_19_R3(EntityType<? extends EnderDragon> entitytypes, Level world) {
        this(world.getWorld());
    }

    public PetEnderDragon_v1_19_R3(World world) {
