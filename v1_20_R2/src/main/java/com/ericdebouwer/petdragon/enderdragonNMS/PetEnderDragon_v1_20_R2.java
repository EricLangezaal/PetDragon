...
public class PetEnderDragon_v1_20_R2 extends EnderDragon implements PetEnderDragon {
    ...
    static {
        ...
        try {
            if (!BuiltInRegistries.ENTITY_TYPE.getOptional(mcKey).isPresent()) {
                ...
                Registry.register(BuiltInRegistries.ENTITY_TYPE, PetEnderDragon.ENTITY_ID,
                        EntityType.Builder.of(PetEnderDragon_v1_20_R2::new, MobCategory.MONSTER).noSummon().build(PetEnderDragon.ENTITY_ID));
                BuiltInRegistries.ENTITY_TYPE.freeze();
            }
            // specialsource does ignore reflection, so non mapped names
            jumpField = LivingEntity.class.getDeclaredField("bj");
            jumpField.setAccessible(true);
            checkWalls = EnderDragon.class.getDeclaredMethod("b", AABB.class);
            checkWalls.setAccessible(true);
            checkCrystals = EnderDragon.class.getDeclaredMethod("gd");
    ...
    public PetEnderDragon_v1_20_R2(EntityType<? extends EnderDragon> entitytypes, Level world) {
        this(world.getWorld());
    }

    public PetEnderDragon_v1_20_R2(World world) {
        ...
        for (Entity entity : list) {
            if (entity instanceof LivingEntity) {
                ...
                if (didMove && ((LivingEntity) entity).getLastHurtByMobTimestamp() < entity.tickCount - 2) {
                    entity.hurt(this.damageSources().mobAttack(this), plugin.getConfigManager().getWingDamage());
    ...
    private void hurt(List<Entity> list) {
        for (Entity entity : list) {
            if (entity instanceof LivingEntity) {
                entity.hurt(this.damageSources().mobAttack(this), plugin.getConfigManager().getHeadDamage());
                this.doEnchantDamageEffects(this, entity);
            }
        }
    }
